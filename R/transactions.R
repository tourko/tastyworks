...
process <- list(
  ...
  expired = function(transactions) {
    ...
    open_gt_close <- options %>%
      dplyr::select(cusip, open_close, quantity) %>%
      dplyr::group_by(cusip, open_close) %>%
    ...
    expired_options <- transactions$option %>%
      dplyr::filter(cusip %in% open_gt_close$cusip, open_close == "OPEN", expiration_date < lubridate::today())
